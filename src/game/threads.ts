...
export function maybeSpawnThreads(s: GameState): void {
  ...
  // Cadencia: como mucho un hilo nuevo cada 4 escenas. El cooldown solo se
  // consume cuando de verdad nace un hilo; intentar un tipo ya agotado no debe
  // silenciar durante cuatro escenas otras historias que sí siguen disponibles.
  const scene = s.sceneCount ?? 0;
