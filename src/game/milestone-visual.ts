...
export function milestoneVisualSpec(share: ShareData): MilestoneVisualSpec {
  ...
  const awardWon = !thirdPartySubject && !formerSubject && !awardNearMiss && ((namedAward.test(subject) && /\b(?:ganas?|gana|ganamos|ganan|conquistas?|conquista|recibes?|recibe|levantas?|levanta|te coronas|eres (?:elegido|elegida)|te (?:conceden|otorgan)|premiado|galardonado)\b/.test(subject)) || /\b(?:ganador|ganadora|vencedor|vencedora)\b.{0,36}\b(?:balon de oro|the best|bota de oro|golden boy)\b/.test(subject));
