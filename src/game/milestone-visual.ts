...
export function milestoneVisualSpec(share: ShareData): MilestoneVisualSpec {
  ...
  const nonTitleChampionLabel = /\bcampeon(?:es|a|as)?\s+(?:(?:de|del)\s+)?(?:(?:torneo|trofeo)\s+(?:de\s+)?)?(?:invierno|pretemporada|amistoso|amistosa|amistosos|amistosas|moral)\b/.test(haystack);
