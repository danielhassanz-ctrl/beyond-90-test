...
function clean(v: string): string { return v.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z]/g, ""); }
export function normalizedNationality(value: string): string { const key = clean(value || "España"); return NORMALIZED[key] ?? (value.trim() || "España"); }
export function confederationFor(nationality: string): Confederation {
  const n = normalizedNationality(nationality);
  if (CONMEBOL.has(n)) return "CONMEBOL"; if (CONCACAF.has(n)) return "CONCACAF"; if (CAF.has(n)) return "CAF";
  if (AFC.has(n)) return "AFC"; if (OFC.has(n)) return "OFC"; return "UEFA";
}
