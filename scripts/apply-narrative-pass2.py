...
if marker not in s:
    raise SystemExit('BEATS marker not found')
if 'const EARLY_VARIETY_STORIES = [' in s:
    raise SystemExit('early variety stories already present')
block = r'''
...
'''
s = s.replace(marker, block + marker + '  ...EARLY_VARIETY_BEATS,\n', 1)
p.write_text(s)

q = Path('src/qa/run.ts')
qs = q.read_text()
old = 'if (card.kind === "arc_beat" && card.data["beatId"] === "beat_grupo_equivocado") rareCount++;'
new = 'if (card.kind === "arc_beat" && (card.data["beatId"] === "beat_grupo_equivocado" || String(card.data["beatId"] ?? "").includes("beat_early_rare_"))) rareCount++;'
if old not in qs:
    raise SystemExit('rare metric target not found')
