...
function resolvePending(s:GameState):GameState{
  ...
  if(s.pending.type==="match")return resolveMatch(s,s.pending.match,s.pending.match.keyMoment?.options[0]?.id);
  ...
  return choice?resolveDynamicCard(s,s.pending,choice.id):advance(s);
}
function runFirstSeason(mode:CareerMode,seed:number){
  const originalRandom=Math.random;Math.random=rng(seed);
  try{
    let s=createGame(player);s.careerSeed=seed;setCareerMode(s,mode);
    const offer=s.offers[0];assert.ok(offer,`${mode}/${seed}: no initial club offer`);s=chooseClub(s,offer.clubId);
...
console.log("RUNTIME_PACING_SMOKE_OK");
