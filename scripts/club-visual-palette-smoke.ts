...
import { clubVisualIdentity, hasReadablePrimary, visualContrast } from "../src/game/club-identity";
...
for (const club of [...CLUB_POOL, ...EURO_POOL]) {
  ...
  } else if (!hasReadablePrimary(identity)) {
    // Exercise the exact production contract instead of maintaining a second
    // contrast implementation in QA that could silently drift from the UI.
    const primaryTextContrast = visualContrast(identity.text, identity.primary);
