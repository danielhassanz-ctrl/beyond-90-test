...
// Keep the physical-Safari diagnostic outside the server-rendered body. Inserting a script
// into the hydrated body changes the prerendered DOM and causes React hydration error #418.
// A head-only listener leaves the body byte-for-byte compatible with the server snapshot and
// still surfaces uncaught startup errors or a persistently blank screen after six seconds.
...
if (!/<\/head>/i.test(html)) throw new Error("Portable HTML has no </head> for Safari diagnostic insertion");
html = html.replace(/<\/head>/i, `${safariDiagnostic}</head>`);
